...
def shuffle(cards):
    ...
    first_slice = list(cards[:int(len(cards)/2)])
    second_slice = list(cards[int(len(cards)/2):])
    shuffled = first_slice + second_slice
    shuffled[::2] = first_slice
    shuffled[1::2] = second_slice
    return shuffled
